...
@api_routing("/search2")
def search_id2():
    client = request.remote_addr + "_" + request.values['u']
    query = request.values["query"]
    (
        artist_count,
        artist_offset,
    ...
    song_offset = int(song_offset) if song_offset else 0
    ytm = YTM()
    results = ytm.search(query, artist_limit=artist_count, album_limit=album_count, song_limit=song_count)
    # root = get_root_folder(mfid)
